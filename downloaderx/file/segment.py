        ...
        if descriptions is None:
            ...
            for description in sorted(list(descriptions), key=lambda d: d.offset):
                if description.offset != offset:
                    raise ValueError(
                        f"except segment offset to be continuous: wanna {offset} but got {description.offset}"
                    )
                if description.completed_length > description.length:
                    raise ValueError(
                        f"segment completed length {description.completed_length} cannot be greater than its length {description.length}"
                    )
                self._nodes.append(
                    _Node(
                        taken=False,
